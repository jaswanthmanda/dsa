...
class Solution(object):
    def minimumCost(self, start, target, specialRoads):
        ...
        # Optional pruning: remove useless special roads
        roads = []
        for x1, y1, x2, y2, c in specialRoads:
            if c < abs(x2 - x1) + abs(y2 - y1):
                roads.append((x1, y1, x2, y2, c))
        ...
        while pq:
            ...
            # 1) Try going directly
            direct = cost + abs(tx - x) + abs(ty - y)
            if dist.get((tx, ty), float("inf")) > direct:
                dist[(tx, ty)] = direct
                heapq.heappush(pq, (direct, tx, ty))

            # 2) Try all special roads
            for x1, y1, x2, y2, c in roads:
                # Move to special road start
                to_start = cost + abs(x1 - x) + abs(y1 - y)
                if dist.get((x1, y1), float("inf")) > to_start:
                    dist[(x1, y1)] = to_start
                    heapq.heappush(pq, (to_start, x1, y1))

                # If we are at the start, take the special road
                if x == x1 and y == y1:
                    to_end = cost + c
                    if dist.get((x2, y2), float("inf")) > to_end:
                        dist[(x2, y2)] = to_end
                        heapq.heappush(pq, (to_end, x2, y2))
