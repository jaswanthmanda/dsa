...
and a 2D string array ingredients. The ith recipe has
the name recipes[i], and you can create it if you have all the
needed ingredients from ingredients[i].
...
class Solution(object):
    def findAllRecipes(self, recipes, ingredients, supplies):
        ...
        for r, ing_list in zip(recipes, ingredients):
            for ing in ing_list:
                if ing not in adjlist:
                    adjlist[ing] = [r]
                else:
                    adjlist[ing].append(r)
                indegree[r] += 1
        ...
        while q:
            ...
            for recipe in adjlist.get(item, []):
                indegree[recipe] -= 1
